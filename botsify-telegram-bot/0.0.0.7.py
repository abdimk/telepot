...
def on_chat_message(msg):
    ...
    if command == '/start':
        markup = InlineKeyboardMarkup(inline_keyboard=[[dict(text='☂️Developer', url='https://t.me/paperextend/'),dict(text='Switch inline', switch_inline_query='initial query')],[]])
        responce = bot.getChat(chat_id)
    ...
    if command =='/radio':
        responce = bot.getChat(chat_id)
        chat_id = 1129016149 #Admins chat_id
        if chat_id == int(responce['id']):
            msg_admin = command
            msg_value = msg_admin.split(' ')
            users =' '
            while True:
                bot.sendMessage(users,f'{msg_value[1]}',reply_markup=None)
'''
    if command =='/random':
